...
- Set the gRPC transaction service to the pending state
- Send several transactions to the pending state
  - The first is a valid transaction
  - The second transaction has the value that exceeds the sender balance
- Check the correct balance of the sender account on the pending state after
  the application of the block
#+BEGIN_SRC fish
...
txSignAndSend $node $sender to 12 $ownerpass

...
- Keccak256 hash function :: The =Hash= type is an alias to =[32]byte=. The
...
- =SigTx= type :: The =SigTx= type embeds the =Tx= type and includes the
  signature byte slice that holds the transaction signature. The string
  representation of the signed transaction is defined to present a transaction
  to end users

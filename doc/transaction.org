...
- =SigTx= type :: The =SigTx= type embeds the =Tx= type and includes the
  transaction signature. The string representation of a signed transaction is
  defined to present the transaction to the end user
...
./bcn tx sign --node $node --from $sender --to to --value 12 \
  --ownerpass $ownerpass
  #+END_SRC

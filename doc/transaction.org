...
- Transaction search :: The transaction search function locates confirmed
  ...
  added to the confirmed state. The transaction search is performed against the
  confirmed state of the blockchain. The difference between the transaction
  search and the subscription to the node event stream is that the node event
  stream allows clients to proactively subscribe to the validated transaction
  event type before transactions are validated and the event will be delivered
  only once, while the transaction search locates transaction on demand after
  that have been validated as many times as required. The transaction search
  locates validated transactions in the confirmed state by the transaction hash,
  by the prefix of the sender account address, by the prefix of the recipient
  account address, and by the prefix of the account involved as a sender or as a
...
transactions on the blockchain. The interface of the service
#+BEGIN_SRC protobuf
message TxSearchReq {
  string Hash = 1;
  string From = 2;
...
- Iterate of each block in the local block store in order. For each block
  - Iterate over each transaction of the confirmed block. For each transaction
    - Search by the transaction hash prefix
      - Send the first transaction that matches the requested transaction hash
        ...
        process
    - Search by the sender, recipient, or account address prefix
      - Send every transaction that matches the search criteria over the gRPC
        server stream and keep searching until all transactions in all blocks
        are searched
#+BEGIN_SRC go
...
- Set up the gRPC server and client
- Search by sender account address

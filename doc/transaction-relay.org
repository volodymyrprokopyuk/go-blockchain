...
- Set up the new node
  ...
  - Synchronize the state on the new node
...
- Wait for the sent transactions to be relayed to the new node and applied to
  the pending state on the new node
- Verify correct balances for the initial owner account on the pending state of
  the new node and the bootstrap node
#+BEGIN_SRC fish
